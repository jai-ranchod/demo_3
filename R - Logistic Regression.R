...
model <- glm(Survived ~ Pclass + SibSp + Parch + LogFare + sex_binary + AgeHigh + AgeMid, data = train, family = "binomial")
vif(model)
...
#outcome.  The default step function, which we use here, is a backstep feature selection function based on the Akaike Information Criterion.
#Given the known instability of the backstep approach to feature selection:
#https://quantifyinghealth.com/stepwise-selection/
#we apply a boostrap method in conjunction with the backstep feature selection approach.
v <- c()
m <- 100
...
{
  set.seed(seed = i+1) #we already used seed = 1 earlier
  traindf <- data.frame()
  ...
  model <- glm(Survived ~ Pclass + SibSp + Parch + LogFare + sex_binary + AgeHigh + AgeMid, data = traindf, family = "binomial")
  model <- step(model, trace = 0)
  
  k <- attr(model$terms, "term.labels")
  v <- c(v,k)
  print(i) #just to track loop progress, feel free to comment out
}


pclass <- sum(v == "Pclass")/m
sibsp <- sum(v == "SibSp")/m
parch <- sum(v == "Parch")/m
logfare <- sum(v == 'LogFare')/m
sex_binary <- sum(v == 'sex_binary')/m
agehigh <- sum(v == 'AgeHigh')/m
agemid <- sum(v == 'AgeMid')/m
...
#We note here that the following features appear in a preponderance of the bootstrap models and should therefore be included
#in the final model:
#AgeHigh
#AgeMid
#sex_binary
#pclass
#sibsp
#Given our historical knowledge of the Titanic disaster it makes sense that age, sex, and cabin class would play a significant
#role in survival probability.We also notice that the presence of siblings and spouses appears to be a significant feature.
#Parch appears in very few models; therefore can exclude these.
#For these purposes, we will also exclude LogFare which appears in fewer than half of the models.

modelfinal <- glm(Survived ~ Pclass + SibSp + sex_binary + AgeHigh + AgeMid, data = train, family = "binomial")
...
#set accessed in the first section and use it to make a prediction on the test set we held out in the beginning.
test <- test %>% mutate(predicted_percent_survival = predict(modelfinal, newdata = test, type = "response"))
test$predicted_percent_survival <- round(test$predicted_percent_survival) #Note that this assumes a cutoff of 0.5.
...
#We then selected our feature set using a combination of bootstrap and backstep approaches.
...
AUC <- sum(s)
AUC
#We note that we have done quite well in terms of AUC.  According the Hosmer,Lemeshow, and Sturdivant (2013, online ISBN: 9781118548387)
#we achieve here "excellent discrimination".
