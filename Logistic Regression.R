...
{
  set.seed(seed = i)
  n <- sample(1:nrow(train),size = 0.8*nrow(train),replace = TRUE)
  
  model <- glm(Survived ~ Pclass + SibSp + Parch + LogFare + sex_binary + AgeHigh + AgeMid, data = train[n,], family = "binomial")
  model <- step(model, trace = 0)
  
  m <- attr(model$terms, "term.labels")
  v <- c(v,m)
  #print(n[1])
}
pclass <- 0
...
#role in survival probability.  It also makes sense that parents and siblings would have an influence.  
#We also notice that the presence of siblings and spouses appears to be a significant feature.
#AgeMid appears in none of our models, and Parch appears in very few; therefore can exclude these.
#For these purposes, we will also exclude LogFare which appears in fewer than half of the models.

modelfinal <- glm(Survived ~ Pclass + SibSp + sex_binary + AgeHigh, data = train, family = "binomial")
